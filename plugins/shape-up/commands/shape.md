...
Before starting, read `docs/concepts/[name]/frame.md` and verify the status is `frame-go`. If the status is `candidate`, tell the user the concept needs framing first. If the document does not exist, tell the user to run `/shape-up:frame` first.
...
When the three properties check passes, write the completed Solution section into `docs/concepts/[name]/shape.md` and set `status: shape-go` in `docs/concepts/[name]/frame.md`.
