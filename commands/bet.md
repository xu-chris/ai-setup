...
Before starting, read `docs/concepts/[name].md` and verify the status is `shape-go`. If the status is `frame-go`, tell the user shaping needs to happen first. If the status is `candidate`, tell the user framing is still needed.
...
When the decision is made, write the Bet section into `docs/concepts/[name].md` and set the status to `bet` or back to `candidate` accordingly.
