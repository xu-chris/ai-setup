...
Before starting, read `docs/concepts/[name].md` and verify the status is `bet`. If the status is `shape-go`, tell the user the concept needs a betting decision first. If earlier, direct the user to the appropriate phase.
...
Write the completed scopes document to `docs/concepts/[name].scopes.md` and update the Build section and status in `docs/concepts/[name].md`.
